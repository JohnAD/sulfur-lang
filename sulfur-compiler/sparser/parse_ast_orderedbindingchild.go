...
func parseAstOrderedBindingChild(cursor *parseCursor, token lexer.Token) error {
	...
	case lexer.TT_IDENT:
		return finishAstOrderedBindingChild(cursor, token, ASTN_IDENTIFIER)
	case lexer.TT_STR_LIT:
		return finishAstOrderedBindingChild(cursor, token, ASTN_STRLIT)
	...
	return fmt.Errorf("unhandled AST_ORDRED_BINDING_CHILD parse of %v", token)
}
